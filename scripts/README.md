...
| `eval-assistant.py` | scores the assistant's behaviour -- tool routing, grounding in the reviewed content, arithmetic, honesty about unverified facts, declined actions, formatting -- against 51 fixed questions, using the app's own prompt, tool schemas and round loop. Run it before and after any change to the prompt, the tools, or the model, and compare with `--baseline`. Also reports the size and cold cost of the fixed prompt prefix. |
